...
public class DataSetLoader {
   ...
   * @return
   */
  public static ModelParallel getRegularGridParallel(int min, int max, int distance){
   ...
   * @return
   */
  public static ModelParallel getRandomGridParallel(int min, int max,int distance){
    ModelParallel result=new ModelParallel();
    Random r=new Random(1);
    for(int i=min;i<max;i+=distance){
      for(int j=min;j<max;j+=distance){
   ...
   * @return
   */
  public static ModelParallel getRandomSetParallel(int min, int max,int size){
    ModelParallel result=new ModelParallel();
    Random r=new Random(1);
    for(int i=0;i<size;i++){
      result.p.add(new Particle(0.5,0,0,min+r.nextInt(max-min)+0.5-r.nextDouble(),min+r.nextInt(max-min)+0.5-r.nextDouble()));
   ...
   * @return
   */
  public static ModelParallel getRandomRotatingGridParallel(double speed,int min, int max,int distance){
    ModelParallel result=new ModelParallel();
    Random r=new Random(1);
    for(int i=min;i<max;i+=distance){
      for(int j=min;j<max;j+=distance){
   ...
   * @return
   */
  public static ModelParallel getElaborateParallel(int min, int max,int distance,double deathRate){
    ModelParallel result=getRandomRotatingGridParallel(0.05d,min,max,distance);
    Random r=new Random(1);
    for(int i=0;i<result.p.size();i+=1){
      if(r.nextDouble()>deathRate){result.p.get(i).mass+=r.nextDouble()/2d;continue;}
      result.p.remove(i);i-=1;
